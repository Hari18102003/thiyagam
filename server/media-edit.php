...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT image FROM Media WHERE id = ?";
    ...
    // Update Media Record
    $updateSQL = "UPDATE Media SET title=?, name=?, date=?, image=? WHERE id=?";
    $updateStmt = $conn->prepare($updateSQL);
    $updateStmt->bind_param("ssssi", $title, $name, $date, $currentImage, $id);

    if ($updateStmt->execute()) {
        echo "<script>alert('Media updated successfully!'); window.location.href='../client/pages/dashboard.php';</script>";
    } else {
