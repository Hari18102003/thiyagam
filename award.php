...
$sql = "SELECT id, title, name, date FROM Award ORDER BY id DESC";
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr>
                                ...
                                <td>{$row['date']}</td>
                                <td>
                                    <a href='add-award.html?id={$row['id']}' class='btn edit-btn'>Edit</a>
                                    <a href='admin/award-delete.php?id={$row['id']}' class='btn delete-btn' onclick='return confirm(\"Are you sure you want to delete this record?\")'>Delete</a>
                                </td>
                              </tr>";
                    }
                } else {
